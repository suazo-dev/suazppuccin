...
if is_vim then require "suazppuccin.lib.vim" end
...
local M = {
	default_options = {
		...
		compile_path = vim.fn.stdpath "cache" .. "/suazppuccin",
...
function M.compile()
	...
	for flavour, _ in pairs(M.flavours) do
		M.flavour = flavour
		require("suazppuccin.lib." .. (is_vim and "vim." or "") .. "compiler").compile(flavour)
	end
...
local function get_flavour(default)
	...
	return flavour or M.options.flavour or vim.g.suazppuccin_flavour or M.options.background[vim.o.background]
end
...
function M.load(flavour)
	...
	f(flavour or M.options.flavour or vim.g.suazppuccin_flavour)
end
...
function M.setup(user_conf)
	...
	if user_conf.auto_integrations == true then
		user_conf.integrations = vim.tbl_deep_extend(
			"force",
			require("suazppuccin.lib.detect_integrations").create_integrations_table(),
	...
	local hash = require("suazppuccin.lib.hashing").hash(user_conf)
...
vim.api.nvim_create_user_command(
	"Catppuccin",
	function(inp) vim.api.nvim_command("colorscheme suazppuccin-" .. get_flavour(inp.args)) end,
	{
		nargs = 1,
...
if vim.g.suazppuccin_debug then
